...
def load_progress():
    if not os.path.exists(PROGRESS_FIle):
        return {"lesson_done": [], "quize_scorse": [], "cards_score": 0}

    with open(PROGRESS_FIle, "r", encoding= "uft-8") as f:
        data = json.load(f)
    return data
...
def save_progress(data):
    with open(PROGRESS_FIle, "w", emcoding = "utf-8") as f:
...
def add_card_seen():
    data = load_progress()
    data["card_seen"] = data["card_seen"] + 1
    save_progress(data)
...
def add_quiz_score(score, total):
    data = load_progress()
    data["quize_scores"].append({"score": score, "total": total})
    save_progress(data)
